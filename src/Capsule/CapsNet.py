...
class CapsNet(tf.keras.Model):
    ...
    def __init__(self, config, **kwargs):
        ...
        if self.config.RECONSTRUCTION_ON:

            # Masking of the output for training purpose
            self.masked = Mask(name='mask')

            # reshaping the capsule layer for the decoder network
            self.secondarycaps = tf.keras.layers.Reshape(target_shape=[-1, len(config.CLASS_NAMES) * capsule_output_dim],
                                                         name='secondary_cap')

            # Decoder network
            self.x_recon1 = tf.keras.layers.Dense(512, activation='relu')
            self.x_recon2 = tf.keras.layers.Dense(1024, activation='relu')
            self.x_recon3 = tf.keras.layers.Dense(config.IMG_HEIGHT * config.IMG_WIDTH, activation='sigmoid')
            self.reshape2 = tf.keras.layers.Reshape(target_shape=[config.IMG_WIDTH, config.IMG_HEIGHT, 1],
                                                    name='out_recon')
